...
def rider_all_data(response:Response,skip: int = 0, limit: int = 25,username=Depends(auth_handler.auth_wrapper)):
        ...
        if limit>0:
            ...
            elif limit > rider_collection.count_documents({}):
                response.status_code = status.HTTP_422_UNPROCESSABLE_ENTITY
            ...
            for collection in rider_collection.find({}).limit(limit).skip(skip):
                ...
                collection["store_id"] = str(collection["store_id"])
                collection["created_by"] = str(collection["created_by"])
                collection["updated_by"] = str(collection["updated_by"])
